...
import Record from "./fastParity_Record";
import useStyles from "../home/style/game";
import BidHistory from "./fastParity_BidHistory";
...
const FastParityGamePage = () => {
  ...
  axios.get(`${URL}/fastParity_getBidHistory/${foundUser.result.id}/`, { headers: { Authorization: AuthStr } })
    .then(response => {
 ...
 const [timerr, setTimerr] = useState(0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTimerr(prevTimer => prevTimer - 1);
    }, 1000);
  ...
  useEffect(() => {
    ...
    if (timerr < 0) {
      setLoader(true)
      axios.get(`${URL}/fastParity_getTimer/`)
      .then(response => {
        var time = response.data[0].date/1000 + 60 ;
        var timeRemaining ;
        ...
        setTimerr(timeRemaining);
        axios.get(`${URL}/fastParity_getRecord/`)
      .then(response => {
  ...
  }, [timerr]);// add variable as dependency
  ...
  const handleFormData = (bidOn, bidAmount) => async (event) =>  {
    ...
    const bidData = {bidOn: bidOn, bidAmount: bidAmount, userId: isAuth.result.id, auth: AuthStr};
    try {
      if(bidHistory !== 'No Data'){
        ...
        if(element.period === (record[0].id + 1)){
          if(element.select.includes('Green') && bidOn.includes('Red')){
            setWarning({...openWarning, open: true, body: 'Be aware that placing bid in opposite colors in same period will not increase your withdrawal balance'});
          }
      ...
      setState({ ...state, bottom: false });
      const { data } = await api.fastParityBidData(bidData);
      dispatch({ type: "FastParityBidData", data: data });
      refreshUserData()
        ...
        >
          ...
          >
            ...
            <Typography align='center'style={{paddingTop: '20px',color: "black",fontSize: '10px',fontWeight: 'bold'}}>
              FastParity 1 Minute Period - {openWinning.result && openWinning.result.id}
            </Typography>
...
If the result shows 5, you will get (98*1.5) 147rupees.</Typography>
      ...
      <Grid
        className={classes.headerFastParity}
        container
      ...
      >
        ...
        gutterBottom
        align="center"
        style={{ paddingTop: "10px", color: '#05c0b8',  fontWeight:  'bold' , fontSize: '18px'}}
      >
        Fast Parity
      </Typography>
        </Grid>
        ...
        <Grid container spacing={0} direction="row" justify="space-between">
          ...
          <Grid item >
            ...
            <Grid container direction="row" justify="center">
              ...
              {timerr < 0 ? '0' : timerr < 10 ? `0${Math.round(timerr - Math.floor(timerr / 60) * 60)}` : Math.round(timerr - Math.floor(timerr / 60) * 60)}              
            </Typography>
              </Box>
      ...
      <Container style={{ paddingTop: "20px" }}>
        <Grid container direction="row" justify="space-between">
          <Grid item>
            <Typography style={{color: "#05c0b8"}}>{'Fast Parity'} Record</Typography>
          </Grid>
          <Link to={`/fastParity/record`} style={{textDecoration: 'none', color: 'black'}}>
          <Grid item>
      ...
      <Container style={{ paddingTop: "20px", paddingBottom: '80px'}}>
        <Grid container direction="row" justify="space-between">
          <Grid item>
            <Typography>{'Fast Parity'} Bid History</Typography>
          </Grid>
          <Link to={`/fastParity/bidHistory`} style={{textDecoration: 'none', color: 'black'}}>
          <Grid item>
            <Typography style={{ color: "grey" }}>more ›</Typography>
          </Grid>
          </Link>
...
export default FastParityGamePage;
