...
const BigSmallFullBidHistory = () => {
  ...
  useEffect(() => {
    ...
    setLoader(true); 
    axios.get(`${URL}/bigsmall_getFullBidHistory/${foundUser.result.id}`, { headers: { Authorization: AuthStr } })
      .then(response => {
         setLoader(false);
           ...
           <Typography style={{paddingTop: '10px', color: "white" }}>Please Wait!</Typography>
            ...
            <Grid container direction="row" justify="" alignItems="center" style={{paddingLeft: '20px',paddingTop: '15px',paddingBottom: '15px',paddingRight: '20px', backgroundColor: 'white'}}>
                ...
                <ArrowBackIosIcon style={{fontSize:'20px'}} onClick={history.goBack} />
                 ...
                 <Grid item xs={4}>                    
                <Typography align="center" >{'Big Small'} Bid History</Typography>
                 </Grid>
            </Grid>
...
export default BigSmallFullBidHistory ; 
