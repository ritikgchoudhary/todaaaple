  ...
  <div class="page">
    <div class="mobileContainer">
      <header class="header">
        ...
        <div class="headerTitle">Today's Promotion Record</div>
      </header>

      <div class="contentWrapper">
        <p class="subtitle">Stats for today's invitations</p>
          ...
          <table class="customTable">
            ...
            <tbody>
              <tr v-for="phone in currentLevelPhones" :key="phone">
                <td>{{ phone }}</td>
                <td class="text-green">₹{{ getLevelData(phone, 'todayRecharge') }}</td>
                <td class="text-red">₹{{ getLevelData(phone, 'todayWithdrawal') }}</td>
                <td class="font-bold">₹{{ getLevelData(phone, 'bidToday') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import { getPromotionNew } from '../api/promotion'

const router = useRouter()
const levelDataNew = ref({})
...
onMounted(async () => {
  ...
  try {
    const response = await getPromotionNew(foundUser.result.id)
    levelDataNew.value = response.data
  } catch (error) {
    router.push('/login')
  } finally {
    loader.value = false
...
const currentLevelPhones = computed(() => {
  const current = levelDataNew.value[`level${level.value}`]
  return current ? Object.keys(current) : []
})

const getLevelData = (phone, field) => {
  try {
    const data = levelDataNew.value[`level${level.value}`]?.[phone]?.[field]
    if (field === 'bidToday') {
