...
const ModifyPassword = () => {
  ...
  const handleChange = (e) => {
    setPasswordData({ ...passwordData, type: 'password', [e.target.name]: e.target.value });
  }

  const handleSubmit = async (e) => {
    setLoader(true);
    const AuthStr = 'Bearer '.concat(isAuth.token);
    setPasswordData({ ...passwordData, auth: AuthStr });
    try {
      const { data } = await api.accountSecurity(passwordData);
      dispatch({ type: "accSeq", data: data });
      history.push("/accountSecurity");
    } catch (error) {
      setLoader(false);
      setDialog({ ...openDialog, open: true, body: error.response?.data?.error || "Error updating password" })
    }
    setLoader(false);
  }
    ...
    <div className={classes.page}>
      <div className={classes.frame}>
        <div className={classes.header}>
          ...
          <Typography className={classes.headerTitle}>Modify Password</Typography>
        </div>
