...
import FastParityPeriod from "../../model/fastParity/bidData.js";
import Manual from "../../model/fastParity/result.js";
import Price from "../../model/fastParity/period.js";
import BidHistory from "../../model/fastParity/bidHistory.js";
import ErrorResponse from "../../utils/error.js";
import Record from "../../model/fastParity/record.js";
...
export const fastParity_bidData = async (req, res, next) => {
  try {
...
export const fastParity_getPricing = async (req, res) => {
...
export const fastParity_setAutoMode = async (req, res) => {
  const auto = req.params.auto;
  await extra.updateOne(
    { id: 1 },
    { "fastParity.auto": auto === "true" ? true : false }
  );
};

export const fastParity_setResult = async (req, res) => {
...
export const fastParity_getTimer = async (req, res) => {
  const lastDate = await Record.find().sort({ date: -1 }).limit(1);

  res.send(lastDate);
};
export const fastParity_getRecordData = (req, res) => {
  Record.find()
    .sort({ date: -1 })
    .limit(10)
    .then((result) => {
...
export const fastParity_getFullRecordData = (req, res) => {
  Record.find()
    .sort({ date: -1 })
    .then((result) => {
      res.send(result.length > 0 ? result : "No Data");
...
export const fastParity_getHistoryData = async (req, res) => {
...
export const fastParity_getFullHistoryData = async (req, res) => {
  var userId = req.params.id;
...
export const fastParity_getCurrentNumber = async (req, res) => {
  const api = req.params.api;
